...
class DouYinVideo(DouYinBaseUploader):
    ...
    async def set_thumbnail(self, page: Page):
        ...
        await page.wait_for_timeout(800)
        confirm_text = page.get_by_text("是否确认应用此封面？", exact=False).first
        if await confirm_text.count() and await confirm_text.is_visible():
            confirm_dialog = confirm_text.locator(
                "xpath=ancestor::div[contains(@class,'dy-creator-content-modal')][1]"
            )
            await confirm_dialog.get_by_role("button", name="确定", exact=True).click()
            await page.wait_for_timeout(500)
        douyin_logger.info(_msg("🥳", "视频封面设置完成"))
        # 新版抖音会保留弹窗节点，只把它隐藏；等待 detached 会在封面已经应用后误报失败。
    ...
    async def upload(self, playwright: Playwright) -> None:
        ...
        await save_draft_only(page, "抖音")
        douyin_logger.success(_msg("🥳", "视频草稿保存成功，未发布"))
...
class DouYinNote(DouYinBaseUploader):
    ...
    async def upload_note_content(self, page: Page) -> None:
        ...
        await save_draft_only(page, "抖音")
        douyin_logger.success(_msg("🥳", "图文草稿保存成功，未发布"))
