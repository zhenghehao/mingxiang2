...
from utils.draft_delivery import save_draft_only
...
class KSBaseUploader(BaseVideoUploader):
    ...
    async def close_guide_overlay(self, page: Page) -> bool:
        """关掉快手的新手引导遮罩（react-joyride）。尽力而为，关不掉也不能挡住上传。

        原来这里直接 `await close_button.click(force=True)`：一旦那三个 Skip
        选择器都没匹配上，Playwright 会按默认超时干等 30 秒再抛异常，整个快手
        任务就死在一个新手引导浮层上（2026-07-26 实测就是这么失败的）。
        现在改成短超时 + 逐个候选试 + Esc 兜底，全都失败也只是往下走 ——
        遮罩顶多挡住一次点击，不该让整条上传前功尽弃。
        """
        joyride_tooltip = page.locator('div[id^="react-joyride-step"] div[role="alertdialog"]')

        try:
            if not (await joyride_tooltip.count() and await joyride_tooltip.first.is_visible()):
        ...
        for selector in selectors:
            try:
                button = page.locator('div[role="alertdialog"]').locator(selector).first
        ...
        try:  # 最后试一下 Esc
            ...
            await joyride_tooltip.wait_for(state="hidden", timeout=3000)
            print("✅ 已用 Esc 关闭 Joyride 遮罩")
            return True
        except Exception:
            print("⚠️ 没能关掉 Joyride 遮罩，继续往下走")
            return False
...
class KSVideo(KSBaseUploader):
    ...
    async def upload(self, playwright: Playwright) -> None:
        ...
        try:
            ...
            await save_draft_only(page, "快手")
            kuaishou_logger.success(_msg("🥳", "视频草稿保存成功，未发布"))
...
class KSNote(KSBaseUploader):
    ...
    async def upload_note_content(self, page: Page) -> None:
        ...
        await save_draft_only(page, "快手")
        kuaishou_logger.success(_msg("🥳", "图文草稿保存成功，未发布"))
