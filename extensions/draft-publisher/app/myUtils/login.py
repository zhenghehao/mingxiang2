...
def storage_state_has_login_data(file_path):
    """本地检查登录文件，避免为了刷新账号列表反复启动浏览器。"""
    try:
        with Path(file_path).open("r", encoding="utf-8") as state_file:
...
# 统一获取浏览器启动配置。草稿版使用普通可视浏览器，不加入规避检测参数。
...
async def douyin_cookie_gen(id,status_queue):
    ...
    async with async_playwright() as playwright:
        ...
        result = storage_state_has_login_data(cookies_dir / f"{uuid_v1}.json")
        if not result:
            status_queue.put("500")
            await page.close()
            await context.close()
